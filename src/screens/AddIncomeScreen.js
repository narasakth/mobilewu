/**
 * AddIncomeScreen.js - หน้าเพิ่มรายรับ
 */
...
import { createIncome, INCOME_CATEGORIES } from '../services/incomeService';
...
const AddIncomeScreen = ({ navigation }) => {
    ...
    const handleSubmit = async () => {
        ...
        setSubmitting(true);
        try {
            await createIncome({ amount: parseFloat(amount), date, category, note: note.trim() || null });
            Alert.alert('สำเร็จ', 'บันทึกรายรับเรียบร้อย', [{ text: 'ตกลง', onPress: () => navigation.goBack() }]);
        } catch (error) {
        ...
        <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
            <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
                ...
                <TouchableOpacity style={[styles.submitButton, submitting && styles.submitButtonDisabled]} onPress={handleSubmit} disabled={submitting} activeOpacity={0.8}>
                    <Text style={styles.submitButtonText}>{submitting ? 'กำลังบันทึก...' : '💵 บันทึกรายรับ'}</Text>
                </TouchableOpacity>
            </ScrollView>
...
const styles = StyleSheet.create({
    ...
    currencySymbol: { fontSize: 24, fontWeight: 'bold', color: '#27ae60', paddingLeft: 16 },
    ...
    categoryButtonSelected: { borderColor: '#27ae60' },
    categoryButtonText: { fontSize: 16, color: '#333' },
    dropdownIcon: { fontSize: 12, color: '#999' },
    noteInput: { minHeight: 80, textAlignVertical: 'top' },
    submitButton: { backgroundColor: '#27ae60', paddingVertical: 16, borderRadius: 8, alignItems: 'center', marginTop: 20 },
    ...
    categoryItemActive: { backgroundColor: '#e8f5e9', borderColor: '#27ae60' },
    categoryIcon: { fontSize: 32, marginBottom: 8 },
    categoryLabel: { fontSize: 14, color: '#666', textAlign: 'center' },
    categoryLabelActive: { color: '#27ae60', fontWeight: '600' },
});

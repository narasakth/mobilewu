/**
 * EditExpenseScreen.js - หน้าแก้ไขรายจ่าย
 */
...
import { updateExpense, EXPENSE_CATEGORIES } from '../services/expenseService';
...
const EditExpenseScreen = ({ navigation, route }) => {
    ...
    const getCategoryDisplay = () => { const cat = EXPENSE_CATEGORIES.find(c => c.id === category); return cat ? `${cat.icon} ${cat.label}` : 'เลือกหมวดหมู่'; };
    ...
    const handleSubmit = async () => {
        ...
        setSubmitting(true);
        try {
            await updateExpense(transaction.id, { amount: parseFloat(amount), date, category, note: note.trim() || null });
            Alert.alert('สำเร็จ', 'แก้ไขรายจ่ายเรียบร้อย', [{ text: 'ตกลง', onPress: () => navigation.goBack() }]);
        } catch (error) {
            ...
            <Modal visible={showCategoryPicker} transparent animationType="slide" onRequestClose={() => setShowCategoryPicker(false)}>
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <FlatList data={EXPENSE_CATEGORIES} keyExtractor={(item) => item.id} renderItem={renderCategoryItem} numColumns={2} contentContainerStyle={styles.categoryList} />
                    </View>
                </View>
            </Modal>
...
const styles = StyleSheet.create({
    container: { flex: 1, backgroundColor: '#f5f5f5' }, scrollView: { flex: 1 }, scrollContent: { padding: 16, paddingBottom: 40 }, section: { marginBottom: 20 }, label: { fontSize: 14, fontWeight: '600', color: '#333', marginBottom: 8 }, input: { backgroundColor: '#fff', borderWidth: 1, borderColor: '#ddd', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 12, fontSize: 16, color: '#333' }, amountContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', borderWidth: 1, borderColor: '#ddd', borderRadius: 8 }, currencySymbol: { fontSize: 24, fontWeight: 'bold', color: '#e74c3c', paddingLeft: 16 }, amountInput: { flex: 1, fontSize: 24, fontWeight: 'bold', color: '#333', paddingHorizontal: 12, paddingVertical: 16 }, dateButton: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#fff', borderWidth: 1, borderColor: '#ddd', borderRadius: 8, paddingHorizontal: 16, paddingVertical: 14 }, dateButtonText: { fontSize: 16, color: '#333' }, dateIcon: { fontSize: 20 }, categoryButton: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#fff', borderWidth: 1, borderColor: '#ddd', borderRadius: 8, paddingHorizontal: 16, paddingVertical: 14 }, categoryButtonSelected: { borderColor: '#e74c3c' }, categoryButtonText: { fontSize: 16, color: '#333' }, dropdownIcon: { fontSize: 12, color: '#999' }, noteInput: { minHeight: 80, textAlignVertical: 'top' }, submitButton: { backgroundColor: '#e74c3c', paddingVertical: 16, borderRadius: 8, alignItems: 'center', marginTop: 20 }, submitButtonDisabled: { backgroundColor: '#bdc3c7' }, submitButtonText: { color: '#fff', fontSize: 18, fontWeight: '600' }, cancelButton: { paddingVertical: 14, borderRadius: 8, alignItems: 'center', marginTop: 12 }, cancelButtonText: { color: '#666', fontSize: 16 }, modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }, modalContent: { backgroundColor: '#fff', borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingBottom: 30, maxHeight: '70%' }, modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: '#eee' }, modalTitle: { fontSize: 18, fontWeight: '600', color: '#333' }, modalClose: { fontSize: 24, color: '#999', padding: 4 }, categoryList: { padding: 12 }, categoryItem: { flex: 1, flexDirection: 'column', alignItems: 'center', justifyContent: 'center', margin: 6, padding: 16, backgroundColor: '#f8f8f8', borderRadius: 12, borderWidth: 2, borderColor: 'transparent' }, categoryItemActive: { backgroundColor: '#fdecea', borderColor: '#e74c3c' }, categoryIcon: { fontSize: 32, marginBottom: 8 }, categoryLabel: { fontSize: 14, color: '#666', textAlign: 'center' }, categoryLabelActive: { color: '#e74c3c', fontWeight: '600' },
});
